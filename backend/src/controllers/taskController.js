...
exports.deleteTask = async (req, res) => {
  try {
    if (req.user.role !== 'HR') {
      return res.status(403).json({ message: 'Доступ заборонено. Тільки HR може видаляти задачі.' });
    }
    ...
    res.status(500).json({ message: 'Помилка при видаленні задачі', error: error.message });
  }
};